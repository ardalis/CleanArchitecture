  ...
  IOptions<MailserverConfiguration> mailserverOptions) : IEmailSender
{
  ...
  public async Task SendEmailAsync(string toAddress, string from, string subject, string body)
  {
    _logger.LogWarning("Sending email to {ToAddress} from {From} with subject {Subject} using {Type}.", toAddress, from, subject, this.ToString());
