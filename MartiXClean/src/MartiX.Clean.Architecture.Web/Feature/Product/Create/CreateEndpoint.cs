...
namespace MartiX.Clean.Architecture.Web.Feature.Product.Create;
...
public class CreateEndpoint(IRepository<ProductEntity> repository) :
  Endpoint<CreateProductRequest,
           Results<Created<ProductRecord>, ValidationProblem, ProblemHttpResult>>
{
  private readonly IRepository<ProductEntity> _repository = repository;
  ...
  {
    var product = ProductEntity.Create(request.Name, request.UnitPrice);
